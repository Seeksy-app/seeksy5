...
export default function BoardWhatsNew() {
  const [readIds, setReadIds] = useState<Set<string>>(new Set());
  const { unreadCount, markAllAsRead } = useUnreadUpdates('board');

  const { data: updates, isLoading } = useQuery({
    queryKey: ['platform-updates', 'board'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('platform_updates')
        .select('*')
        .contains('visibility', ['board'])
  ...
  // Fetch read status
  useQuery({
    queryKey: ['user-update-reads', 'board'],
    queryFn: async () => {
    ...
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Key updates and strategic improvements</p>
        </div>
      </div>
