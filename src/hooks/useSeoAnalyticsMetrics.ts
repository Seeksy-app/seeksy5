...
export function useSeoListMetrics(routePaths: string[], timeRange: TimeRange = '7d') {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const gscByPage = new Map<string, { clicks: number; impressions: number; ctrs: number[]; positions: number[] }>();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [gsc7d, gsc28d, ga47d, ga428d] = await Promise.all([
        (supabase as any).from('gsc_page_daily').select('clicks, impressions, ctr, position').eq('workspace_id', DEFAULT_WORKSPACE_ID).eq('page', normalized).gte('date', date7d),
        (supabase as any).from('gsc_page_daily').select('clicks, impressions, ctr, position').eq('workspace_id', DEFAULT_WORKSPACE_ID).eq('page', normalized).gte('date', date28d),
        (supabase as any).from('ga4_page_daily').select('sessions, engagement_rate').eq('workspace_id', DEFAULT_WORKSPACE_ID).eq('page_path', normalized).gte('date', date7d),
        (supabase as any).from('ga4_page_daily').select('sessions, engagement_rate').eq('workspace_id', DEFAULT_WORKSPACE_ID).eq('page_path', normalized).gte('date', date28d)
      ]);
      ...
      function aggregateGsc(rows: any[]): GscMetrics | null {
        ...
        return { clicks, impressions, ctr: impressions > 0 ? (clicks / impressions) * 100 : 0, position: positions.length > 0 ? positions.reduce((a, b) => a + b, 0) / positions.length : 0 };
      }
      ...
      function aggregateGa4(rows: any[]): Ga4Metrics | null {
        ...
        return { users: Math.round(sessions * 0.8), sessions, views: Math.round(sessions * 1.3), engagementRate: rates.length > 0 ? rates.reduce((a, b) => a + b, 0) / rates.length : 0 };
      }

      return {
        '7d': { gsc: aggregateGsc(gsc7d.data || []), ga4: aggregateGa4(ga47d.data || []) },
        '28d': { gsc: aggregateGsc(gsc28d.data || []), ga4: aggregateGa4(ga428d.data || []) }
      };
    },
